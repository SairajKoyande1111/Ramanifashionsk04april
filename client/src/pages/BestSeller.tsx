...
export default function BestSeller() {
  ...
  if (selectedFabrics.length > 0) queryParams.append("fabric", selectedFabrics.join(","));
  if (selectedColors.length > 0) queryParams.append("color", selectedColors.join(","));
  if (selectedOccasions.length > 0) queryParams.append("occasion", selectedOccasions.join(","));
  ...
  const priceRangeParams = new URLSearchParams({ isBestseller: "true" });
  if (selectedCategories.length > 0) priceRangeParams.append("category", selectedCategories.join(","));
  if (selectedFabrics.length > 0) priceRangeParams.append("fabric", selectedFabrics.join(","));
  if (selectedColors.length > 0) priceRangeParams.append("color", selectedColors.join(","));
  if (selectedOccasions.length > 0) priceRangeParams.append("occasion", selectedOccasions.join(","));

  const { data: priceRangeData } = useQuery<{ minPrice: number; maxPrice: number }>({
          ...
          {activeFiltersCount > 0 && (
            <div className="flex items-center gap-2 mt-4 flex-wrap">
              ...
              {selectedColors.map((col) => (
                <Button key={col} size="sm" variant="secondary" onClick={() => toggleColor(col)}>
                  {col} <X className="h-3 w-3 ml-1" />
                </Button>
              ))}
              <Button size="sm" variant="ghost" onClick={clearAllFilters} data-testid="button-clear-filters">
                Clear All
              </Button>
            </div>
          )}
          ...
          <div className="lg:col-span-3">
            {isLoading ? (
              ...
              <div className="text-center py-20">
                <p className="text-muted-foreground text-lg mb-4">No best seller products found</p>
        ...
        <div className="grid grid-cols-2 gap-px">
          <Sheet open={showMobileSort} onOpenChange={setShowMobileSort}>
            <SheetTrigger asChild>
              <Button variant="ghost" className="h-14 rounded-none flex items-center justify-center gap-2" data-testid="button-mobile-sort">
          ...
          <Sheet open={showFilters} onOpenChange={setShowFilters}>
            <SheetTrigger asChild>
              <Button variant="ghost" className="h-14 rounded-none flex items-center justify-center gap-2" data-testid="button-mobile-filter">
            ...
            <SheetContent side="bottom" className="h-[80vh] overflow-y-auto">
              ...
              <div className="mt-6 flex gap-3">
                <Button variant="outline" className="flex-1" onClick={() => { clearAllFilters(); setShowFilters(false); }}>
                  Clear All
                </Button>
